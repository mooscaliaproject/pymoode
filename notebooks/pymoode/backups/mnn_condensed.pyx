...
cdef c_calc_mnn_iter(
    ...
    for i in calc_items:
        ...
        if last_n == -1:
            ...
            last_d = D[c_square_to_condensed(i, last_n, N)]
        ...
        for j in H:
            ...
            #Calculate distance if elements satisfy rule
            elif (D[c_square_to_condensed(i, j, N)] <= last_d):
                ...
                for m in range(M):
                    ...
                    #If smaller than last, replace it
                    elif (D[c_square_to_condensed(i, j, N)] <= D[c_square_to_condensed(i, Mnn[i, m], N)]):
...
cdef c_calc_d(double[:] d, int[:, :] Mnn, vector[double] D, cpp_set[int] calc_items, int M):
    ...
    for i in calc_items:
        ...
        d[i] = 1
        for m in range(M):
            d[i] = d[i] * D[c_square_to_condensed(i, Mnn[i, m], Mnn.shape[0])]
